...
**词法单元**：亦称单词/词素，源程序中有特定含义的字符序列，如`if`、`+`。

**词法记号**：亦称记号，由**记号名**（和**属性值**）构成的一元组`<name>`或二元组`<name, attribute>`。

* 记号名代表**一类词法单元**的抽象（概括性）名字，如程序中的标识符的记号名为**`id`**。

* 对于只有一种属性的记号而言，属性值不是必须的，可以用一元组表示，比如说`if`。

**模式**：词法记号的**模式**描述了**该种词法记号所对应的词法单元**应有的形式。对于较简单的词法记号，比如说**`<if>`**，其模式就是字符串`"if"`，而其他一些词法记号的模式则更复杂，一般由一定的字符串匹配方式定义，比如说标识符的定义规则。

### 2.1.2 词法记号的属性

一个词法记号的记号名可能对应**多个词法单元**。当这种情况出现时，为了**消除歧义**，需要在词法记号中加入属性值概念，记录这一“附加信息”。
...
**串**：某字母表中的字符构成的有限序列，比如说`USTC`、`114514`。若某串仅含某字母表中的符号，则称其为**某字母表上的串**。
...
**语言**：某字母表上的串的集合，比如说{`USTC`, `114514`}、{`0011101`, `ε`}，其中的元素称作该语言的**句子**或**字**。

对于**语言**，可以定义并、连接、闭包和正闭包运算：

* **并运算**：`L1 ∪ L2 = {w | w ∈ L1 或 w ∈ L2}`
* **连接运算**：`L1L2 = {w1w2 | w1 ∈ L1 且 w2 ∈ L2}`
* **闭包运算**： `L^* = ∪(i=0, ∞) L^i`，即零个或多个`L`中的串相互连接构成的集合。
* **正闭包运算**：`L^+ = ∪(i=1, ∞) L^i`，即一个或多个`L`中的串相互连接构成的集合。

### 2.2.2 正规式

**正规式/正则表达式**：用于描述某种语言的，形式化的表达式。

正规式的定义规则如下：

1. 归纳奠基：空串`ε`是一个正规式，它代表语言`{ε}`。对于字母表中的每个符号`a`，`a`也是一个正规式，它代表语言`{a}`。
2. 归纳步骤：若`r1`和`r2`是正规式，代表的语言分别为`L(r1)`和`L(r2)`，则以下式子也是正规式：
   * `r1 | r2`：`r1`和`r2`的并集，代表语言`L(r1) ∪ L(r2)`
...
另外，在正规式中，某些结构重复出现，为方便起见赋予其特定的缩写形式：

1. **一个或多个实例**：用`+`表示，`r+ = rr*`，所以`r+`代表的语言为`L(r)^+`。
2. **零个或一个实例**：用`?`表示，`r? = r | ε`，所以`r?`代表的语言为`L(r) ∪ {ε}`。
...
* 有些状态还带有一个星号`*`，它代表输入指针在进行接下来的识别任务之前将回退一个字符。

  ![image-20240909112657429](Chapter2.assets/image-20240909112657429.png)

## 2.3 有限自动机

有限自动机可以识别正规式描述的语言，其接受一个串作为输入，在有限个状态之间通过状态转移函数进行状态间的转换，最终输出为该输入是否为某语言的句子的判断结果。
在编译原理中，有限自动机分为**不确定的有限自动机（NFA）**和**确定的有限自动机（DFA）**，下面分别进行介绍。

### 2.3.1 不确定的有限自动机（下称NFA）

NFA的本质是一个数学模型，可以写成一个五元组`N = <S, Σ, move(·,·), s_0, F>`，其中：
...
* `ε-closure(s)`：状态`s`的ε闭包，即从NFA的状态`s`出发，通过**任意个**ε转移，可以到达的**所有状态**的集合。
* `ε-closure(T)`：状态集合`T`的ε闭包（`∪{ε-closure(s) | s ∈ T}`），即状态集合`T`中的每个状态的ε闭包的**并集**。
* `move(T, a)`：状态集合`T`中的每个状态（`∪{move(s, a) | s ∈ T}`），即状态集合`T`中所有状态通过输入符号`a`能够转移到的状态的集合的并集。

然后，如下执行算法：

1. 令`A <- {ε-closure(s_0)}`，其中`s_0`为NFA的初始状态。
2. 标记`A`，`A`为DFA的初始状态。
3. 对每一个输入符号`a`，执行以下步骤：
   1. 令`B <- ε-closure(move(A, a))`。
   2. 若`B`不在已标记状态中，则标记`B`。
   3. 令`DFA`的状态转移函数`move(A, a) <- B`。
...
如果一个DFA的转换函数不是全函数，可以引入一个“死状态”`sd`：

* 对所有的输入符号，`sd`都转换到`sd`本身。
* 如果在`move`函数中，没有状态`s`与符号`a`对应的状态，那么令`move`将`<s,a>`映射到状态`sd`。
